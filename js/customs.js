+function ($) {
    ...
    $(window).on('load',function(){ 
    	if( get_window_sizes('w') <= bc_config.breakpoints.sm){
    		//bs_scroll_to('#propiedades', -60);  
    	} 
    });


    var wpbc_custom_select = $('.qtranxs_widget select');
    wpbc_custom_select.selectpicker({
    	'style':'btn-white'
    });
...
+function ($) {
    ...
    $(".dropdown-select .dropdown-menu .dropdown-item").on('click', function(){
      ...
      if ($(this).data('reload-select')) {
        var elementId = $(this).data('reload-select');
        var groupId = $(this).data('reload-select-group');
        var target = $(elementId).children('div').data('input-target');
        $(elementId).find('div.subgroup').hide();
        if (groupId) {
            $('#'+groupId).show(); 
        } 
        var firstElement = $(elementId).find('.dropdown-menu > a').first();
        $(elementId).find('.dropdown-select-value').data('value', $(firstElement).data('value'));
        $(elementId).find('.dropdown-select-value').html($(firstElement).text());
        $(target).val($(firstElement).data('value'));
      }
